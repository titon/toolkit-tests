...
try {
    ...
    if (empty($_SERVER['HTTP_X_HUB_SIGNATURE']) || empty($_ENV['SECRET_TOKEN'])) {
        throw new Exception('Missing secret token.', 400);
    }

    if ($_SERVER['HTTP_X_HUB_SIGNATURE'] !== 'sha1=' . hash_hmac('sha1', $_POST['payload'], $_ENV['SECRET_TOKEN'], false)) {
